...
export function TaskCard({ task, overdue }: { task: Task; overdue?: boolean }) {
  const inner = (
    <div className={`bg-white rounded-xl border px-4 py-3.5 hover:shadow-sm transition-all ${
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {task.project ? (
            ...
            <p className="text-slate-300 text-xs mt-0.5">Tarea personal</p>
          )}
          {task.briefId && (
  ...
  if (!task.projectId) return <div className="active:opacity-80 cursor-default">{inner}</div>

  return (
    <Link href={`/projects/${task.projectId}`} className="block active:bg-gray-50">
      {inner}
    </Link>
  )
}
