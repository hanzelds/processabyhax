...
import { LockOpen, Trash2, Loader2, AlertTriangle, ScrollText, ExternalLink } from 'lucide-react'
          ...
          >
            {editing ? (
              ...
              <div>
                ...
                {task.description && (
                  <p className="text-slate-500 text-sm mb-3 leading-relaxed">{task.description}</p>
                )}

                {/* Scripts linked via brief */}
                {task.brief?.scripts && task.brief.scripts.length > 0 && (
                  <div className="mb-4 space-y-1.5">
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Guiones</p>
                    {task.brief.scripts.map(s => (
                      <Link
                        key={s.id}
                        href={`/content/scripts/${s.id}`}
                        onClick={closeModal}
                        className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-50 border border-slate-200 hover:border-teal-300 hover:bg-teal-50/40 transition-all group"
                      >
                        <ScrollText className="w-3.5 h-3.5 text-teal-600 shrink-0" />
                        <span className="flex-1 text-sm text-slate-800 font-medium truncate group-hover:text-teal-700">{s.title}</span>
                        ...
                        <ExternalLink className="w-3 h-3 text-slate-300 group-hover:text-teal-500 shrink-0" />
                      </Link>
                    ))}
                  </div>
                )}
