...
vehiclesRouter.get('/:id', async (req: Request, res: Response) => {
  const item = await prisma.vehicle.findFirst({ where: { id: req.params.id, organizationId: getOrgId(req) }, select: VEHICLE_SELECT })
...
vehiclesRouter.post('/', isAdminOrLead, async (req: Request, res: Response) => {
  ...
  const item = await prisma.vehicle.create({
    data: { name, plate, type, brand: brand || null, model: model || null, year: year ? Number(year) : null, color: color || null, status: status || 'AVAILABLE', notes: notes || null, imageUrl: imageUrl || null, organizationId: getOrgId(req) },
...
vehiclesRouter.patch('/:id', isAdminOrLead, async (req: Request, res: Response) => {
  const existing = await prisma.vehicle.findFirst({ where: { id: req.params.id, organizationId: getOrgId(req) }, select: { id: true } })
